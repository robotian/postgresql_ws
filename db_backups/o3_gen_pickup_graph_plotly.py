...
def insert_graph_to_db(G):
    ...
    try:
        ...
        # 2. Truncate both tables if they already contained old data
        cursor.execute("TRUNCATE TABLE graph_node, graph_edge;")
        ...
        for node_id, data in G.nodes(data=True):
            node_records.append((
                ...
                data.get('y', 0.0), 
                data.get('theta', 0.0), 
                data.get('obj_id', None) # obj_id is None for non-pickup nodes
            ))
        ...
        cursor.executemany("""
            INSERT INTO graph_node (id, node_type, x, y, theta, obj_id)
            VALUES (%s, %s, %s, %s, %s, %s)
        """, node_records)

        cursor.executemany("""
            INSERT INTO graph_edge (source_id, target_id, weight, edge_type)
            VALUES (%s, %s, %s, %s)
        """, edge_records)
        ...
        cursor.close()
        conn.close()
        print(f"Successfully inserted {len(node_records)} nodes and {len(edge_records)} edges into the database.")
...
def build_navigation_graph(object_data):
    ...
    # 4. Connect Via Nodes
    def get_n_closest(target_node_id, candidates_ids, n=2):
        sorted_candidates = sorted(candidates_ids, key=lambda c: calc_dist(G.nodes[target_node_id], G.nodes[c]))
        return sorted_candidates[:n]

    for ent_id in entering_nodes_ids:
        for via_id in get_n_closest(ent_id, via_ccw_ids, n=2) + get_n_closest(ent_id, via_cw_ids, n=4):
    ...
    for ext_id in exiting_nodes_ids:
        for via_id in get_n_closest(ext_id, via_ccw_ids, n=2) + get_n_closest(ext_id, via_cw_ids, n=4):
...
def visualize_graph(G, objects):
    ...
    # ----------------------------------------
    # 1. Edges (Curved Bezier Lines + Hover Layers)
    # ----------------------------------------
    edge_ax, edge_ay = [], []
    ...
    for u, v, data in G.edges(data=True):
        ...
        if dist == 0: 
            dist = 0.01  # Avoid division by zero for overlapping nodes
            # continue
        
        # Determine Control Point for curve (offset normal to line)
        mid_x, mid_y = (x1 + x2) / 2.0, (y1 + y2) / 2.0
        nx_norm = (y2 - y1) / dist
        ny_norm = -(x2 - x1) / dist
        
        curve_factor = 0.15 # Determines how far out the curve bows
        c_x = mid_x + nx_norm * (dist * curve_factor)
        c_y = mid_y + ny_norm * (dist * curve_factor)
        
        # Generate 15 points along the Quadratic Bezier Curve
        num_points = 15
        curve_x, curve_y = [], []
        ...
        # Tangent vector at the end of the curve (to angle the arrowhead correctly)
        theta = math.atan2(y2 - c_y, x2 - c_x)
        hl = 0.05 # Arrowhead size
        ...
        # Draw the curve, then trace out the V for the arrowhead
        edge_ax.extend(curve_x + [h1x, x2, h2x, None])
        edge_ay.extend(curve_y + [h1y, y2, h2y, None])
        
        # Determine Hover target exactly on the middle of the arc (t=0.5)
    ...
    # Plot the invisible Edge hover targets (placed directly on the curve)
    fig.add_trace(go.Scatter(
    ...
    for node_id, data in G.nodes(data=True):
        ...
        # Node Hover Data
        node_hover_x.append(nx_val + math.cos(ntheta) * 0.05)
    ...
    # Plot invisible Node hover targets
    fig.add_trace(go.Scatter(
    ...
    # ----------------------------------------
    fig.update_layout(
        title="Husky + Kinova Nav2 Complex Waypoint Graph (Curved Edges)",
