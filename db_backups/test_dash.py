...
from dash import Dash, dcc, html, Input, Output, no_update, Patch
...
def build_navigation_graph(object_data):
    ...
    objects = [] 
    node_id_counter = 1
    entering_nodes_ids, exiting_nodes_ids = [], []
...
def visualize_graph_dash(G, objects):
    fig = go.Figure()
    
    # Store mapping between hover points and edge geometry for Dash Callback
    edge_map = {} 
    hover_idx = 0
    ...
    for u, v, data in G.edges(data=True):
        ...
        curve_factor = 0.15 
        c_x = mid_x + nx_norm * (dist * curve_factor)
        c_y = mid_y + ny_norm * (dist * curve_factor)
        
        num_points = 15
        curve_x, curve_y = [], []
        for i in range(num_points + 1):
            t = i / num_points
            px = (1-t)**2 * x1 + 2*(1-t)*t * c_x + t**2 * x2
            py = (1-t)**2 * y1 + 2*(1-t)*t * c_y + t**2 * y2
            curve_x.append(px)
            curve_y.append(py)
            
        theta = math.atan2(y2 - c_y, x2 - c_x)
        hl = 0.05 
        ...
        geom_x = curve_x + [h1x, x2, h2x, None]
        geom_y = curve_y + [h1y, y2, h2y, None]
        edge_ax.extend(geom_x)
        edge_ay.extend(geom_y)
        
        hover_px = 0.25 * x1 + 0.5 * c_x + 0.25 * x2
        hover_py = 0.25 * y1 + 0.5 * c_y + 0.25 * y2
        
        edge_hover_x.append(hover_px)
        edge_hover_y.append(hover_py)
        ...
        # Save geometry corresponding to this specific hover index
        edge_map[hover_idx] = {
            'curve_x': geom_x, 'curve_y': geom_y,
            'u_x': x1, 'u_y': y1, 'v_x': x2, 'v_y': y2
        }
        hover_idx += 1

    fig.add_trace(go.Scatter(
        x=edge_ax, y=edge_ay, mode='lines',
        line=dict(color='rgba(0, 0, 255, 0.4)', width=1, shape='linear'),
        name='Directed Edge', showlegend=True, hoverinfo='none'
    ))
    
    # Crucial: Give this trace a name and customdata to identify it in the callback
    fig.add_trace(go.Scatter(
        x=edge_hover_x, y=edge_hover_y, mode='markers',
        marker=dict(size=12, color='rgba(0,0,0,0)'),
        hoverinfo='text', hovertext=edge_hover_text,
        customdata=list(range(len(edge_hover_x))),
        showlegend=False, name="Edge Info"
    ))

    # 2. Objects & Nodes Geometry
    ...
    node_arrows, node_text_x, node_text_y, node_text = [], [], [], []
    ...
    for node_id, data in G.nodes(data=True):
        ...
        # Add a slight offset to the hover point for better detection
        node_hover_x.append(nx_val + math.cos(ntheta) * 0.15)
        node_hover_y.append(ny_val + math.sin(ntheta) * 0.15)
    ...
    # Add invisible hover points for nodes to trigger the callback
    fig.add_trace(go.Scatter(
    ...
    dock_x_arrows, dock_y_arrows, dock_center_x, dock_center_y = [], [], [], []
    for dx, dy, dt in DOCK_POSES:
    ...
    fig.add_trace(go.Scatter(
        ...
        marker=dict(color='gray', size=8), name="Docking Station", showlegend=True, hoverinfo='none'
    ))

    # ==========================================
    # Highlight Traces for Dash Callback
    # These MUST be the last two traces added to the figure.
    # ==========================================
    # Highlighted Thick Edge (Index -2)
    fig.add_trace(go.Scatter(
        x=[], y=[], mode='lines',
        line=dict(color='red', width=5),
        showlegend=False, hoverinfo='skip', name='Highlight Edge'
    ))
    # Highlighted Thick Nodes (Index -1)
    fig.add_trace(go.Scatter(
        x=[], y=[], mode='markers',
        marker=dict(size=24, color='rgba(0,0,0,0)', line=dict(color='red', width=4)),
        showlegend=False, hoverinfo='skip', name='Highlight Nodes'
    ))

    fig.update_layout(
        title="Husky + Kinova Nav2 Complex Waypoint Graph (Hover for Highlight)",
...
def main():
    ...
    # Generate the figure and our edge_map reference table
    fig, edge_map = visualize_graph_dash(G, objects)

    # Initialize Dash App
    app = Dash(__name__)
    
    app.layout = html.Div([
        # clear_on_unhover clears the highlight when the mouse leaves the target
        dcc.Graph(id='network-graph', figure=fig, clear_on_unhover=True) 
    ])
    ...
    def update_hover_highlight(hoverData):
        # We use Patch() to only update the modified traces, making it extremely fast
        patched_fig = Patch()
        
        # If nothing is hovered, clear the highlight x/y arrays
        if hoverData is None:
            patched_fig['data'][-2]['x'] = []
            patched_fig['data'][-2]['y'] = []
            patched_fig['data'][-1]['x'] = []
            patched_fig['data'][-1]['y'] = []
            return patched_fig

        pt = hoverData['points'][0]
        
        # Make sure the user is hovering over the "Edge Info" trace 
        # (which we tagged with a customdata array)
        if 'customdata' not in pt:
            return no_update
            
        edge_idx = pt['customdata']
        hovered_edge = edge_map[edge_idx]
        
        # Patch the Highlight Edge trace (Second to last trace: Index -2)
        patched_fig['data'][-2]['x'] = hovered_edge['curve_x']
        patched_fig['data'][-2]['y'] = hovered_edge['curve_y']
        
        # Patch the Highlight Nodes trace (Last trace: Index -1)
        patched_fig['data'][-1]['x'] = [hovered_edge['u_x'], hovered_edge['v_x']]
        patched_fig['data'][-1]['y'] = [hovered_edge['u_y'], hovered_edge['v_y']]
        
        return patched_fig

    # Run the interactive app locally
    print("Starting Interactive Dash Application. Open the local address in your browser...")
    app.run(debug=True)  # <-- Changed this line from app.run_server to app.run
